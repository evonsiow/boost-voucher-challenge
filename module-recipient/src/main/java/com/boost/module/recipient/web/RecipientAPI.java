...
public interface RecipientAPI {
    @PostMapping
    ResponseEntity<ApiResponse<RecipientResponseVM>> createRecipient(@Valid @RequestBody RecipientCreationRequestVM recipientCreationRequestVM);

    @GetMapping
    ResponseEntity<ApiResponse<List<RecipientResponseVM>>> getAllRecipients();

    @GetMapping
    ResponseEntity<ApiResponse<RecipientResponseVM>> getRecipientDetails(
